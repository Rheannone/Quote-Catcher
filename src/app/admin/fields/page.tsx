...
// Renders the built-in form sections exactly as they look to customers,
// but non-interactive and slightly faded.

function StaticInput({ placeholder }: { placeholder: string }) {
  return (
    <div className="form-input text-gray-300 truncate">{placeholder}</div>
  );
}

function StaticTextarea({ placeholder, tall }: { placeholder: string; tall?: boolean }) {
  return (
    <div className={`form-input text-gray-300 ${tall ? "min-h-[80px]" : "min-h-[56px]"}`}>
      {placeholder}
    </div>
  );
}

function StaticLabel({ label, required }: { label: string; required?: boolean }) {
  return (
...
function StaticField({
  ...
  return (
    <div>
      <StaticLabel label={label} required={required} />
      {children}
    </div>
  );
}

function StaticRadioGroup({ label, options, required }: { label: string; options: string[]; required?: boolean }) {
  return (
    <div>
      <StaticLabel label={label} required={required} />
      <div className="flex gap-6 mt-1">
        {options.map((o) => (
          <label key={o} className="flex items-center gap-2 text-sm text-gray-300">
            <span className="w-4 h-4 rounded-full border-2 border-gray-200 inline-block shrink-0" />
            {o}
          </label>
        ))}
      </div>
...
function StaticCheckboxGroup({ label, options, required }: { label: string; options: string[]; required?: boolean }) {
  return (
    <div>
      <StaticLabel label={label} required={required} />
      <div className="flex flex-wrap gap-4 mt-1">
        {options.map((o) => (
          <label key={o} className="flex items-center gap-2 text-sm text-gray-300">
            <span className="w-4 h-4 rounded border border-gray-200 inline-block shrink-0" />
            {o}
          </label>
        ))}
      </div>
...
function StaticSection({ title, badge, children }: { title: string; badge?: string; children: React.ReactNode }) {
  return (
    <section className="bg-white rounded-2xl shadow-sm p-6 space-y-5 select-none pointer-events-none opacity-50">
      <div className="flex items-center justify-between">
        <h2 className="section-heading !mb-0">{title}</h2>
        {badge && (
          <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 bg-gray-100 rounded-full px-2.5 py-1">
            {badge}
          </span>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col h-full border-l border-gray-200 bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
        ...
        <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 text-xl leading-none transition" aria-label="Close">
          &times;
        </button>
      </div>
          ...
          <div>
            ...
            {optList && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {optList.map((o) => (
                  <span key={o} className="text-xs bg-gray-100 text-gray-600 rounded-full px-2.5 py-0.5">
                    {o}
                  </span>
                ))}
              </div>
        ...
        <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4">
          ...
          {fieldType === "checkbox" ? (
            ...
            <div className="pointer-events-none select-none">
              ...
              {fieldType === "textarea" && (
                <div className="form-input min-h-[56px] text-gray-300 text-sm">Your answer here&hellip;</div>
              )}
              {fieldType === "select" && (
                <div className="form-input text-gray-400 flex items-center justify-between">
                  <span>{parseOptions(options)?.[0] || "Select an option\u2026"}</span>
                  <svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
              )}
              {fieldType === "number" && <div className="form-input text-gray-300">0</div>}
              {fieldType === "text" && <div className="form-input text-gray-300">Your answer here&hellip;</div>}
            </div>
          )}
        </div>
      </div>
...
export default function FieldsPage() {
  ...
  const openAdd     = () => { setSelectedId(null); setAddingNew(true); };

  return (
    <div className="flex items-start">
      {/* ── LEFT: full form preview ───────────────────────────────────────── */}
      <div className="flex-1 min-w-0 px-4 py-8">
        <div className="max-w-xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-lg font-black uppercase tracking-widest text-brand">Form Fields</h1>
          ...
          <StaticSection title="Contact Information" badge="Built-in">
            <div className="grid grid-cols-2 gap-5">
              <StaticField label="First Name" required><StaticInput placeholder="Jane" /></StaticField>
              <StaticField label="Last Name" required><StaticInput placeholder="Smith" /></StaticField>
            </div>
            <StaticField label="Band / Company"><StaticInput placeholder="The Rolling Stones" /></StaticField>
            <div className="grid grid-cols-2 gap-5">
              <StaticField label="Email Address" required><StaticInput placeholder="jane@example.com" /></StaticField>
              <StaticField label="Phone" required><StaticInput placeholder="(412) 555-0100" /></StaticField>
            </div>
            <StaticField label="Website"><StaticInput placeholder="https://yourband.com" /></StaticField>
          </StaticSection>

          {/* ── STATIC: Project Details ──────────────────────────────────── */}
          <StaticSection title="Project Details" badge="Built-in">
            <StaticField label="Project Description" required>
              <StaticTextarea placeholder="Describe your project, event, or order…" tall />
            </StaticField>
            <StaticField label="Deadline"><StaticInput placeholder="mm/dd/yyyy" /></StaticField>
            <StaticRadioGroup label="Shipping or Local Pick-up" options={["Pick-up", "Shipping"]} required />
          </StaticSection>

          {/* ── STATIC: Print Specifications ────────────────────────────── */}
          <StaticSection title="Print Specifications" badge="Built-in">
            <StaticField label="What are we printing?" required>
              <StaticInput placeholder="T-shirts, hoodies, posters, etc." />
            </StaticField>
            <StaticField label="How many pieces?" required>
              <StaticInput placeholder="50" />
            </StaticField>
            <StaticCheckboxGroup
              label="Print Locations"
              options={["Front", "Back", "Left Sleeve", "Right Sleeve", "Other"]}
              required
            />
            <StaticField label="Preferred apparel brand" required>
              <StaticInput placeholder="Gildan, Comfort Colors, Bella+Canvas, No preference…" />
            </StaticField>
            <StaticRadioGroup
              label="Do you have print-ready artwork?"
              options={["Yes", "No", "In Progress"]}
              required
            />
            <StaticField label="Additional details or questions">
              <StaticTextarea placeholder="Anything else we should know…" />
            </StaticField>
          </StaticSection>

          {/* ── EDITABLE: Additional Information (custom fields) ─────────── */}
          <div>
            {/* "Your fields go here" divider */}
            <div className="flex items-center gap-3 mb-4">
              <span className="flex-1 h-px bg-brand-accent/30" />
            ...
            <section className="bg-white rounded-2xl shadow-sm p-6 space-y-5">
              <div className="flex items-center justify-between">
                <h2 className="section-heading !mb-0">Additional Information</h2>
                <span className="text-[10px] font-bold uppercase tracking-widest text-brand-accent bg-brand-accent/10 rounded-full px-2.5 py-1">
                  Editable
                </span>
              </div>
              ...
              {fields.length === 0 ? (
                <div
                  className="text-center py-10 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 cursor-pointer hover:border-brand-accent hover:text-brand-accent transition"
                  onClick={openAdd}
                >
                  <p className="text-3xl font-thin mb-2">+</p>
                  <p className="font-semibold">No custom fields yet</p>
                  <p className="text-sm mt-1">Click to add your first question</p>
                </div>
              ) : (
      ...
      <div className={`shrink-0 overflow-hidden transition-all duration-300 ${panelOpen ? "w-[340px]" : "w-0"}`}>
        <div className="w-[340px] sticky top-[57px] h-[calc(100vh-57px)] flex flex-col">
          {panelOpen && (
            <EditorPanel
